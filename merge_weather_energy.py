# merge_weather_energy.py
# ------------------------------------------------------------
# Join multiple daily weather Excel files with an hourly electricity dataset.
# - Weather files: daily values (e.g., Rainfall, Max/Min Temp, Sun Exposure) with columns like Year, Month, Day/Date.
# - Electricity file: hourly readings per day for 49 households; date columns may be 'aggregate_year/month/day' etc.
# The script:
#   1) Reads and harmonises date columns with flexible detection.
#   2) Merges all weather files into a single daily table (wide columns per variable).
#   3) Replicates daily weather to hourly (0..23) for each date.
#   4) Reads the electricity file, extracts date + hour, and preserves household columns.
#   5) Inner-joins electricity with hourly weather on [date, hour], keeping only matching rows.
#   6) Writes merged data to CSV/Parquet.
#
# Usage:
#   - Edit the CONFIG section: point to your local file paths.
#   - Run:  python merge_weather_energy.py
#
# Notes:
#   - Assumes each weather file has exactly one measurement column (apart from date columns).
#     If a weather file has multiple measurement columns, they will all be merged (wide).
#   - Handles date columns in many forms: Year/Month/Day, Year/Month/Date, or a single yyyymmdd.
#   - For electricity: tries 'timestamp', 'datetime', 'hour', 'time', or infers hour by row order (0..23).
#   - Daylight Saving Time (DST): this script treats dates as naive (no timezone) and creates 24 hours per day.
#     This matches many metering exports; adjust if your data truly has 23/25-hour days on DST transitions.
#
# ------------------------------------------------------------
...
# Optional: sheet names if not first sheet / or specify per file via dict below
# Example: WEATHER_SHEETS = {"rainfall_2022.xlsx": "Sheet1", ...}
WEATHER_SHEETS: Dict[str, str] = {}
ELECTRICITY_SHEET = "Source Table"

# Optional: Give a friendly variable name per weather file (else inferred from column header)
# Example: WEATHER_LABEL_HINTS = {"rainfall_2022.xlsx": "rainfall_mm", "tmax_2023.xlsx": "tmax_c"}
WEATHER_LABEL_HINTS: Dict[str, str] = {}

# Optional: set the household ID column name if you know it.
# The script will auto-detect among common options if this remains None.
