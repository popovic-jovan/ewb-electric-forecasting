"""Batch-train SARIMAX daily models for every meter (except exclusions)."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train SARIMAX daily models for all meters.")
    parser.add_argument(
        "--dataset-config",
    ...
    parser.add_argument(
        "--model-config",
        default="configs/model/sarimax_daily.yaml",
        help="Path to SARIMAX model configuration YAML.",
    )
    parser.add_argument(
        "--output-root",
        default="models",
...
def train_all(args: argparse.Namespace) -> None:
    ...
    print(f"Training SARIMAX daily for {len(meters)} meters...")
